...
- **Sender Forensics Card:** in v1, GTR-native. Was briefly slated to move to v1.x as Lumen-only — that decision was reversed in Phase 0 because GTR `reporters/summary` and `owners/summary` provide the data natively. See SPEC §10. Implementation note: the sender drawer itself is LOCAL aggregation from `record.complaints`, not a per-sender API call — the GTR API has no per-sender filter (probed negatively 2026-04-15). Cross-watchlist correlation is the killer feature and is computable client-side.
- **GTR pagination contract (locked 2026-04-15 after live verification):** first page uses `/requests/summary?size=N&domain=X`. Subsequent pages use `/requests/summary/page?size=N&domain=X&p=<cursor>` — endpoint path suffix `/page`, param name `p=` (NOT `?start=`). Pagination metadata positional shape is `[prev, next, last, currentPage, totalPages]` — the **next-page cursor is at index 1**, the last-page cursor is at index 2. Reading `[2]` as next (as an earlier Phase 3 implementation did) causes zero progression beyond page 1. End-of-list is detected by `entries.length < size` OR `currentPage >= totalPages` OR `nextCursor === cursor`. If a future change touches `src/shared/gtr-client.ts: fetchGtrComplaintsPage`, this contract must be preserved. See SPEC §6 Q2 for the full live-verified walk against zippyshare.com.
- **Repo & first release (locked 2026-04-15):** the public repository is [github.com/investblog/dmca-watch](https://github.com/investblog/dmca-watch) under the `investblog` GitHub account, MIT license, homepage `https://dmca.cam`. First beta release `v0.1.0-beta.1` was published via GitHub Actions CI on 2026-04-15 with all four browser zips (chrome-mv3, edge-mv3, firefox-mv2, sources). The release workflow (`.github/workflows/release.yml`) auto-detects prerelease via `contains(github.ref_name, '-')` — tags with a `-` suffix such as `v0.1.0-beta.1` or `v0.2.0-rc.1` become GitHub prereleases; clean tags like `v0.1.0` or `v1.2.3` become standard releases. Do not change this rule without the lead's approval.
